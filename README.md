...
>
> Video Demo: https://drive.google.com/file/d/1TMxu2I3TYscFCtAcNT5QLE4Ek_BhBrnJ/view?usp=sharing 
---
## 🎮 How to Play
...
- 🟥 **Circular ball sprite** with radius-based pixel masking for authentic round appearance
- 🟦 **24 colorful bricks** arranged in 3 rows (8 columns each) with RGB gradient:
  - Red top row, Green middle row, Blue bottom row
  - Black border separation for crisp visual definition
- ⬜ **Dynamic paddle** that follows player input with smooth horizontal movement
- 🖼️ **Custom start screen** and **game over screen** loaded from MIF (Memory Initialization Files)
- 📊 **Real-time score display** on 7-segment HEX displays showing bricks destroyed (0-24)

### 🕹️ **Game Mechanics**
- ⌨️ **PS/2 Keyboard Interface** for real-time player control:
...
- **7 independent state machines** working in parallel:
  1. `game_state_controller` - Master game flow (START → PLAYING → GAME_OVER → RESETTING)
...
#### 💾 **Memory Architecture**
- **On-chip ROM** (altsyncram) for storing 160×120 MIF images:
  - `startscreen.mif` - Welcome screen graphics
  - `gameover.mif` - Game over overlay
- **4×4 pixel scaling** to fill 640×480 screen (hardware upscaler)
- **25-bit brick state register** tracks alive/destroyed status for all bricks

### 🚀 **Performance Optimizations**
- ⚡ **60 FPS game tick** generated by hardware divider from 50MHz system clock
- 🔧 **Gated write signals** during game over prevent unnecessary VGA operations
- 🎯 **Single-cycle collision detection** using parallel comparator arrays
- 🧩 **Resource-efficient sprite rendering** using mathematical circle equations instead of image storage
...
| **Ball Size** | 16×16 pixels (circular mask) |
| **Paddle Size** | 80×10 pixels |
| **Brick Size** | 80×20 pixels (with 2px border) |
| **Total Bricks** | 24 (8 columns × 3 rows) |
...
├── game_state_controller     // Master FSM
├── game_tick_timer            // 60Hz clock divider
├── paddle_controller          // Paddle physics
├── ball                       // Ball physics & collision logic
├── paddle_display             // Paddle VGA renderer
├── ball_display               // Ball VGA renderer
├── brick_display              // Brick array renderer + collision detector
├── startscreen_display        // Welcome screen overlay
├── gameover_display           // Game over screen overlay
├── clear_display              // Screen wipe utility
├── startscreen_mem (ROM)      // Start screen image storage
├── gameover_mem (ROM)         // Game over image storage
└── vga_adapter                // VGA signal generator
```

---
<img width="300" height="600" alt="ChatGPT Image Nov 26, 2025, 07_46_02 PM" src="https://github.com/user-attachments/assets/3b1a3d1a-55a7-470a-8460-8cebd35a8da1" />
